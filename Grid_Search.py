...
H_Measured_1pz = np.array(H_Measured_1pz) #Measured H(z) divided by (1+z). Converting to array. 
...
def ChiSqr(x):
    ...
    H_With_EDM = Output(H_Measured_Redshift) #H(z) from cascade
    #Code below is to handle the two values for H that have slightly different upper and lower error bars. 
...
print(" ")
Gammas = np.linspace(10**(-18), 10**(-15), 40)
zbreaks = np.linspace(0, 20, 40)
fractions = np.linspace(0, 0.25, 20)

ChiSqrVal = 999999999 #Initializing as a very large value
BestGamma = 0 #Initializing these as zero
BestzBreak = 0
BestFraction = 0

for index1 in range(len(Gammas)):
    print(index1) #So we can see progress. 
    GammaVal = Gammas[index1]
    for index2 in range(len(zbreaks)):
        zbreakVal = zbreaks[index2]
        for index3 in range(len(fractions)):
            fractionVal = fractions[index3]
            x = np.array([GammaVal, zbreakVal, fractionVal])
            ChiSqrValNew = ChiSqr(x)
            if ChiSqrValNew < ChiSqrVal:
                ChiSqrVal = ChiSqrValNew
                BestGamma = GammaVal
                BestzBreak = zbreakVal
                BestFraction = fractionVal 
